...
class NeuralNetwork:
    def __init__(self, x_train, y_train, iterations,lr):
        ...
        self.w2   = np.random.normal(0.01, 1, (10,64)) *0.01
        self.b1   = np.random.normal(0.01, 1, (64,1)) *0.01
        self.b2   = np.random.normal(0.01, 1, (10,1)) *0.01
    ...
    def feedforward(self):
        ...
        self.A2 = self.get_softmax(self.Z2,'false')             #A2 = sigmoid(Z2)
        loss = self.get_sigmoid_loss(self.A2,self.y_train_to_go,'false')
        return loss
    ...
    def backprop(self):
        ...
        self.dj_Z2 = self.get_sigmoid_loss(self.A2, self.y_train_to_go,'true')
        self.dZ2_dw2 = self.A1
        self.delta2 = self.dj_Z2
        ...
        self.w1 -= self.lr*self.dj_dw1
        self.w2 -= self.lr*self.dj_dw2
        self.b1 -= self.lr*self.dj_db1
        self.b2 -= self.lr*self.dj_db2
    ...
    def get_softmax(self, Z, backward):
    ...
    def get_sigmoid_loss(self, A, y_gt, backward):
        if backward == 'false':
            loss = -(1.0/y_gt.shape[1]) * np.sum(y_gt*np.log(A+1e-8))
        else:
            #loss = -(1.0/self.m) * np.sum(y_gt/A - (1-y_gt)/(1-A))
            loss = A - y_gt
        return loss
    ...
    def training(self):
        ...
        for i in range(self.epoch):
            ...
            y_train_predicted = np.argmax(self.A2, axis=0)
            y_train_predicted = one_hot_encode(y_train_predicted, digits)
            acc_train = self.get_accuracy(y_train_predicted,self.y_train_to_go)
            ...
            loss_val = self.get_sigmoid_loss(y_val_predicted,y_val,'false')
            ...
            print("Epoch" + str(i) + ": training accuracy " + str(acc_train)+ ": validation accuracy " + str(acc_val) + '\n' +" training loss: "+ str(loss_train)+", validation loss:" + str(loss_val) + '\n')
    ...
    def classify(self,x_test):
        ...
        y_test_predicted = self.get_softmax(np.dot(self.w2,A) + self.b2,'false')
        y_test_predicted = np.argmax(y_test_predicted, axis=0)
        y_test_predicted = one_hot_encode(y_test_predicted, digits)

        return y_test_predicted

    def get_accuracy(self,y_test_predicted,y_test):
        acc = (1.0/y_test.shape[1])* np.sum(y_test * y_test_predicted)
        return acc
